...
const addProductToCart = async (user, productId, quantity) => {
  ...
  cart.cartItems.forEach((ele)=>{
    //console.log(typeof ele.product._id.toString())
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  cart.cartItems.forEach((ele, index)=>{
    // console.log("Element of array", ele.product._id)
    // console.log("Element of array", typeof(parseInt(ele.product._id)),typeof parseInt(productId))
    // console.log("IF Condition", ele.product._id === productId)
    if (ele.product._id.toString() === productId){
      cartIndex = index;
    }
  })
...
const checkout = async (user) => {
  ...
  let addressIsNotSet = await user.hasSetNonDefaultAddress()
  //console.log("Address is Set", addressIsSet)
  if (!addressIsNotSet){
    throw new ApiError(httpStatus.BAD_REQUEST, "Address is not set")
  }
  cart.cartItems.forEach((ele)=>{
    totalCartCost += ele.product.cost * ele.quantity
  })
  if (totalCartCost > user.walletMoney){
    ...
    try{
      cart.cartItems = []
      await cart.save()
      user.walletMoney -= totalCartCost
      await user.save()
    }catch (err){
      console.log(err)
