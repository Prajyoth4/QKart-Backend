...
const generateAuthTokens = async (user) => {
  ...
  let expires = new Date(today.getTime() + config.jwt.accessExpirationMinutes * 60000); // Date object
  let token = generateToken(user._id, Math.floor(Date.now() / 1000) + config.jwt.accessExpirationMinutes * 60, tokenTypes.ACCESS, config.jwt.secret)
